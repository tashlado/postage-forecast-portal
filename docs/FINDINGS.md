...
- **The Apps Script performance basics are already right.** No `flush()` calls anywhere. No
  ...
  is a ranged `setValues`. `prewarmSheetCache_` collapses many tab reads into one HTTP call
  with two documented fallbacks. `Utilities.formatDate` was already hunted down and replaced
  with plain JavaScript after it was measured costing 2.6s of a 3s page load.
...
| **S9** | permissions & data safety | Low | S | `utils.js:14`, `migrate.js:20` | Two production spreadsheet IDs hardcoded and committed to the repository. **Consequence: not secret in themselves, but they pin the code to one environment — there is no way to point a copy at a test sheet without editing source.** |
