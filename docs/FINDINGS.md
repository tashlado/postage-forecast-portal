...
|---|---|---|---|---|---|
| **C1** | correctness bug | High | S | `rates.js:76,172`, `mixes.js:559`, `actuals.js:34`, `structure.js:147` | ✅ **Fixed in Phase 1.** Update paths authorise the **incoming** foreign key, not the one on the row being edited. Sending `{id: <a rate outside my scope>, modellingId: <one I can edit>}` passes the check and rewrites that row, re-parenting it. Delete paths get this right — they read the row first. **Consequence: a scoped user can silently overwrite and steal rows belonging to segments they cannot see.** Fixed by `assertCanEditRowOwner_` (`auth.js`), which reads the row's *current* `High_Level_ID`/`Modelling_ID` from the execution cache and asserts on it, and is now called at all five update sites — `saveBaseRate` (via `writeBaseRate_`), `saveSurchargeRate`, `saveLetterParcelMix`, `saveActual`, `saveModellingId`. It is a no-op on create and defers to the caller's own "no longer exists" error when the row is gone. Placed as the first statement inside each lock, ahead of `closePrecedingPeriod_`, so a refusal leaves nothing half-written. `saveHighLevelId` needed no change — the row it edits *is* its own owner, and it already asserted on `p.id`. Fixed per call site rather than in a shared writer, because the shared writer is **M1**'s job in Phase 8. |
...
| **C2** | correctness bug | Med | S | `rates.js:452` → `rates.js:85` | ✅ **Fixed in Phase 1.** Nested `withLock_`: `bulkRateChange` takes the script lock, then calls `saveBaseRate`, whose own `finally { releaseLock() }` frees it. **Consequence: everything after the first route in a bulk change runs unprotected, so a concurrent write can interleave halfway through.** Fixed by splitting the unlocked body out as `writeBaseRate_(p, perms)`: `saveBaseRate` is now just prewarm + permissions + `withLock_(writeBaseRate_)`, and `bulkRateChange` calls `writeBaseRate_` directly from inside its own lock. The `authorise → closePrecedingPeriod_ → assertNoOverlap_ → write` order is unchanged, and validation moved inside the lock along with the rest of the body. |
...
| **C8** | correctness bug | Low | S | `audit.js:26` `logAudit_` | ✅ **Fixed in Phase 1.** Computes `Log_ID` as max+1 then `appendRow`, frequently **outside** any lock — notably on every DENIED path. **Consequence: concurrent executions can produce duplicate `Log_ID`s. Harmless today since nothing keys on it, but it is presented as an identifier.** Fixed with a Script Property counter (`reserveAuditIds_`/`nextAuditId_` in `audit.js`) rather than a lock: `logAudit_` runs *inside* write paths that already hold the script lock, so taking it here would release it mid-write — reintroducing **C2** in the logger. The property is seeded from the sheet's current maximum on first use so IDs continue rather than restarting at 1, and is keyed by spreadsheet ID because the target sheet is switchable since Phase 0 while Script Properties belong to the script project. IDs are reserved in blocks (8 for single writes, `rows.length` for `recordChangesBatch_`), so a grid write costs one property round-trip; a part-used block leaves a harmless gap. `migrate.js`'s `logAction_` — the **M7** duplicate of `logAudit_` — was moved onto the same counter, since a second max+1 allocator would have reissued IDs the counter had already handed out. `testAuditTrail()` now reports and asserts that the IDs it writes are unique and ascending, judged against the rows it wrote rather than the whole tab so pre-fix duplicates do not fail it. |
...
✅ **Fixed in Phase 1** — the description below is the defect as found; see the register row
for what the fix does.
...
What Phase 1 actually did: the *check* is shared (`assertCanEditRowOwner_`, one function in
`auth.js`), but it is still **called** from five places, because collapsing the five writers
into one `saveDatedRow_` is M1's much larger change and belongs to Phase 8. So the finding is
closed and the duplication it was a symptom of is not.
